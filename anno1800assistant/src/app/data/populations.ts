...
export class PopulationLevel extends PopulationLevelRaw {
  ...
  GetPopulation(factories: Factory[]): number {
    ...
    const enabledOutputProductIDs = {};
    for (const factory of factories) {
      if (factory.Enabled) {
        enabledOutputProductIDs[factory.Outputs[0].ProductID] = true;
      }
    }

    for (const input of this.Inputs) {
      if (enabledOutputProductIDs[input.ProductID]) {
        supplyWeight += input.SupplyWeight;
      }
    }
  ...
  GetProductRequirement(productID: number): number {
    const input = this.Inputs.filter(i => i.ProductID === productID)[0];
    if (this.Inputs.filter(i => i.ProductID === productID)[0]) {
